...
describe('useProjectStore', () => {
  ...
  it('renaming a folder rewrites every descendant path', () => {
    const store = useProjectStore.getState();
    store.openFile('/src/components/Scene3D.tsx');
    ...
    expect(files['/src/components/Scene3D.tsx']).toBeUndefined();
    expect(files['/src/widgets/Scene3D.tsx']).toBeDefined();
    expect(files['/src/widgets/Scene3D.tsx'].parentId).toBe('/src/widgets');
    expect(files['/src/widgets'].children).toContain('/src/widgets/Scene3D.tsx');
    ...
    expect(useProjectStore.getState().openTabIds).toContain('/src/widgets/Scene3D.tsx');
    expect(useProjectStore.getState().activeFileId).toBe('/src/widgets/Scene3D.tsx');
  });
  ...
  it('deleting a folder removes the whole subtree', () => {
    const store = useProjectStore.getState();
    store.openFile('/src/components/Header.tsx');

    useProjectStore.getState().deleteFile('/src/components');

    const files = useProjectStore.getState().projects[0].files;
    expect(files['/src/components']).toBeUndefined();
    expect(files['/src/components/Header.tsx']).toBeUndefined();
    expect(files['/src/components/Scene3D.tsx']).toBeUndefined();
    expect(useProjectStore.getState().openTabIds).not.toContain('/src/components/Header.tsx');
  });
  ...
  it('marks a file unsaved on edit and clears the flag on save', () => {
    useProjectStore.getState().updateFileContent('/src/App.tsx', 'const x = 1;');

    let file = useProjectStore.getState().projects[0].files['/src/App.tsx'];
    expect(file.content).toBe('const x = 1;');
    expect(file.isUnsaved).toBe(true);
    expect(useProjectStore.getState().gitStatus.unstaged).toContain('/src/App.tsx');

    useProjectStore.getState().saveFile('/src/App.tsx');
    file = useProjectStore.getState().projects[0].files['/src/App.tsx'];
    expect(file.isUnsaved).toBe(false);
    expect(file.content).toBe('const x = 1;');
  });

  it('saveAllFiles clears every dirty flag', () => {
    useProjectStore.getState().updateFileContent('/src/App.tsx', 'a');
    useProjectStore.getState().updateFileContent('/README.md', 'b');

    useProjectStore.getState().saveAllFiles();

    const files = useProjectStore.getState().projects[0].files;
    expect(files['/src/App.tsx'].isUnsaved).toBe(false);
  ...
  it('closing the active tab activates a neighbouring tab', () => {
    ...
    store.openFile('/src/components/Header.tsx');
    expect(useProjectStore.getState().activeFileId).toBe('/src/components/Header.tsx');

    useProjectStore.getState().closeTab('/src/components/Header.tsx');
    const state = useProjectStore.getState();
    expect(state.openTabIds).not.toContain('/src/components/Header.tsx');
