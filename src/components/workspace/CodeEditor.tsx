...
const EditorSurface: React.FC = () => {
  ...
  const handleMount: OnMount = useCallback(
    (editorInstance, monaco) => {
      ...
      // Red Noir editor theme.
      monaco.editor.defineTheme('codespace-red-noir', {
        base: 'vs-dark',
        inherit: true,
        rules: [
          { token: 'comment', foreground: '6b7280', fontStyle: 'italic' },
          { token: 'keyword', foreground: 'ef233c', fontStyle: 'bold' },
          { token: 'string', foreground: 'f4a2ab' },
          { token: 'number', foreground: 'ff8fa3' },
          { token: 'type', foreground: 'e4e4e7' },
          { token: 'function', foreground: 'ff6b81' },
        ],
        colors: {
          ...
          'editorIndentGuide.activeBackground': '#ef233c',
          'editor.selectionBackground': '#ef233c40',
          'editorLineNumber.foreground': '#3f3f46',
          'editorLineNumber.activeForeground': '#ef233c',
        },
      });
      monaco.editor.setTheme('codespace-red-noir');
          ...
          return (
            <div
              ...
              className={`flex items-center gap-2 px-3 py-2 text-xs border-r border-outline-variant/10 cursor-pointer min-w-[120px] max-w-[220px] group transition-colors ${
                isTabActive
                  ? 'bg-background border-t-2 border-t-primary text-white font-medium'
