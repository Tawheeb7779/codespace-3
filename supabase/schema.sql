...
-- Backup snapshots taken from the Security & Backup panel.
...
CREATE INDEX IF NOT EXISTS workspace_snapshots_project_id_idx ON public.workspace_snapshots (project_id);
...
DROP POLICY IF EXISTS "Members can view workspace snapshots" ON public.workspace_snapshots;
DROP POLICY IF EXISTS "Developers, admins, and owners can create/delete workspace snapshots" ON public.workspace_snapshots;
...
CREATE POLICY "workspace_snapshots_select"
  ON public.workspace_snapshots FOR SELECT
  TO authenticated
  USING (public.can_read_project(project_id));

CREATE POLICY "workspace_snapshots_insert"
  ON public.workspace_snapshots FOR INSERT
  TO authenticated
  WITH CHECK (public.can_write_project(project_id) AND user_id = auth.uid());

CREATE POLICY "workspace_snapshots_delete"
